...
function semParser(file) {
  var gpsEpoch = arguments.length <= 1 || arguments[1] === undefined ? null : arguments[1];
  var year = arguments.length <= 2 || arguments[2] === undefined ? new Date().getUTCFullYear() : arguments[2];
  var errorOnCountDiff = arguments.length <= 3 || arguments[3] === undefined ? false : arguments[3];
  ...
  var match = /^\s*(\d{1,4})\s+(\d+)\s*/.exec(lines[1]);

  var gpsWeekRelative = parseInt(match[1], 10);

  if (gpsEpoch === null) {

    var gpsWeekStartOfYear = Math.floor((Date.UTC(year, 0, 1) - gpsTimeStart) / (7 * 86400 * 1e3));

    var gpsEpochProposal = 0;
  ...
  var gpsWeek = gpsWeekRelative + gpsEpoch * 1024;
  var toa = parseInt(match[2], 10);
  ...
  var satCount = parseInt(lines[0].slice(0, 2), 10);
